...
**`batch_transfer_task` 表：** 存储批量转账任务的主信息。
...
**`batch_transfer_item` 表：** 存储每个批量转账任务中的详细转账项。
...
**索引考虑：**
  ...
  * `batch_transfer_task`: `(status, created_at)` 用于快速查询不同状态的任务。
  * `batch_transfer_item`: `(task_id)` 用于快速查询某个任务下的所有转账项。
